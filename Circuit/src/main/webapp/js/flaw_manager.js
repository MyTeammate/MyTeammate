		...
		columns : [ [ 
            ...
            {field : 'operation',title : '操作',width : 170,align : 'center',
				...
				if(row.taskStatus=='待分配'){
					oper='<span><a href="javaScript:lookInspection('+row.id+')">查看</a>｜'
					+'<a href="javaScript:allotInspection('+row.id+')">分配任务</a>｜'
					+'<a href="javaScript:updateInspection('+row.id+')">修改</a>｜'
					+'<a href="javaScript:cancelEminate('+row.id+')">取消</a></span>';
					...
					+'<a href="javaScript:updateInspection('+row.id+')">审查</a>｜'
					+'<a style="color:#CDC5BF">取消</a></span>';
