...
<body>
	<div style="padding-left: 20px; padding-top: 15px">消缺任务管理>>消缺任务执行与回执>>消缺回执录入修改</div>
	<hr />
	...
	<input value="${save_eliminateId}" id="return_save_eliminateId" type="hidden"/>
	<table class="hovertable" style="border: 1px solid #ccc;margin-left: 120px;margin-top:20px;width:800px;height:300px;">
	   ...
	   <c:forEach items="${userList}" var="l">
		...
		<tr  >
			...
			<td style="border: 1px solid #ccc;">
			<textarea cols="30" rows="5" id="return_success_desc">${seeEliminate.taskCompletionDesc}</textarea></td>
		</tr>
		<tr  >
	...
	<a href="javascript:onclick=update_return_save_receipt()" class="easyui-linkbutton"
	...
	<p><textarea cols="123" rows="5" id="return_work_record">${seeEliminate.extensionRecord}</textarea></p>
	<p style="font-weight: bold;">工作终结报告</p>
	<p><textarea cols="123" rows="5" id="return_final_report">${seeEliminate.finalReport}</textarea></p>
	</div>
